      ...
      this.items.map(
        item =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              // Create fallback canvas image if load fails
              const fb = document.createElement('canvas');
              fb.width = 512;
              fb.height = 512;
              const fctx = fb.getContext('2d')!;
              fctx.fillStyle = '#182350';
              fctx.fillRect(0, 0, 512, 512);
              fctx.fillStyle = '#AFD2FA';
              fctx.font = 'bold 36px monospace';
              fctx.textAlign = 'center';
              fctx.fillText(item.title || 'PORTAL', 256, 256);
...
const defaultItems: MenuItem[] = [
  {
    ...
    description: 'Autonomous Pathfinding & Emission Control'
  }
];
...
export default function InfiniteMenu({
  ...
  return (
    <div className="relative h-full w-full overflow-hidden select-none" style={{ backgroundColor }}>
      <canvas
        ...
        <>
          ...
          <div
            className={`
              ...
              ${
                isMoving
                  ? 'opacity-0 -translate-x-6 duration-100'
                  : 'opacity-100 translate-x-0 duration-500'
              }
            `}
          >
            {activeItem.tag && (
              <span className="inline-block px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider bg-[#EAF4FE] text-[#182350] border border-[#AFD2FA] mb-2 shadow-xs">
                {activeItem.tag}
              </span>
            )}
            <h2 className="font-extrabold text-2xl sm:text-4xl lg:text-5xl text-[#182350] tracking-tight drop-shadow-sm max-w-[12ch]">
              {activeItem.title}
            </h2>
          </div>
          ...
          <div
            ...
            `}
          >
            <p className="max-w-[18ch] sm:max-w-[22ch] text-sm sm:text-base lg:text-lg font-sans text-[#3F4654] font-medium leading-relaxed drop-shadow-xs text-right">
              {activeItem.description}
            </p>
          </div>

          {/* Center-Bottom Action Pill / Circle Launch Button */}
          <div
            ...
            className={`
              ...
              bg-[#182350] hover:bg-[#233372]
              text-white
              border-2 border-[#AFD2FA]
              rounded-full
              cursor-pointer
              shadow-xl
              transition-all
              ...
              ${
                isMoving
                  ? 'translate-y-8 opacity-0 pointer-events-none duration-100 scale-90'
                  : 'translate-y-0 opacity-100 pointer-events-auto duration-500 scale-100'
              }
            `}
          >
            <span className="text-xs font-mono font-bold uppercase tracking-wider">
              Launch {activeItem.title}
            </span>
            <span className="text-[#AFD2FA] text-base font-bold">↗</span>
          </div>
        </>
      )}
    </div>
